...
class TNN(nn.Module):
    def __init__(self, tn):
        ...
        self.W = nn.Parameter(torch.randn(tn, 10, requires_grad=True))  
...
for fold, (train_idx, val_idx) in enumerate(skf.split(train_test.cpu().numpy(), labels_np)):
    ...
    VNN1 = HNN(EMBEDDING_DIM+10, EMBEDDING_DIM+10).to(device)
    ENN1 = HNN(EMBEDDING_DIM+10, EMBEDDING_DIM+10).to(device)
    VNN2 = HNN(EMBEDDING_DIM+10, EMBEDDING_DIM+10).to(device)
    FNN = HNN(EMBEDDING_DIM+10, 1).to(device)
